...
class FourInLineTreeMinMaxEvaluation:
	...
	def create_tree(self):
		...
		with MyProcessPoolExecutor() as executor:
			finished = False
			while not finished:
				while self.nodes_to_do:
					executor.submit(self.expand_node, self.nodes_to_do.pop(0))
				finished = executor.pool_usage == 0 and not self.nodes_to_do

	def expand_node(self, parent_node):
		def create_new_node(move):
			new_state = FourInLine(deepcopy(parent_node.board))
			new_state.move(move, new_state.current_player)
			parent_node.next.append(new_state)
			return new_state
		...
		if parent_node.lvl < self.MAX_LVL:
			for new_node in create_new_nodes(parent_node):
	...
	def print_positions(self, game_state=None, all_=False):
		...
		if not all_:
			...
			for next_state in game_state.next:
				self.print_positions(next_state, all_)
